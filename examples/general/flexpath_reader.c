...
int flexpath_reader (MPI_Comm adios_comm, char * source) 
{
    int         rank, size, j;
    int         NX, NY, NZ; 
    double      *t;
    ...
    sprintf(file_name, "adios_%s_%s", source, my_name);
    /* schedule_read of a scalar. */    
    int test_scalar = -1;
    ADIOS_FILE* afile = adios_read_open(file_name, 
	...
	adios_schedule_read (afile,
        ...
        printf("%s Rank=%d: test_scalar: %d step: %d, t[0,5+x] = [", my_name, rank, test_scalar, ii);
        for(j=0; j<nelem; j++) {
            printf(", %6.2f", t[j]);
        }
        printf("]\n\n");
